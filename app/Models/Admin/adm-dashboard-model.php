...
class DashboardModel
{
    ...
    public function getStats(): array
    {
        ...
        if ($row = $this->conn->query("SELECT COUNT(*) c FROM users WHERE role='student' AND is_active=1")->fetch_assoc()) {
            $stats['students'] = (int) $row['c'];
        }
        if ($row = $this->conn->query("SELECT COUNT(*) c FROM users WHERE role='adviser' AND is_active=1")->fetch_assoc()) {
            $stats['advisers'] = (int) $row['c'];
        }
        if ($row = $this->conn->query("SELECT COUNT(*) c FROM teams WHERE status='active'")->fetch_assoc()) {
            $stats['activeGroups'] = (int) $row['c'];
        }
        if ($row = $this->conn->query("SELECT COUNT(*) c FROM teams WHERE status='archived'")->fetch_assoc()) {
        ...
        ";
        if ($row = $this->conn->query($pendingArchiveQuery)->fetch_assoc()) {
            $stats['pendingArchive'] = (int) $row['c'];
        }
        if ($row = $this->conn->query("SELECT ROUND(AVG(progress)) c FROM milestones")->fetch_assoc()) {
    ...
    public function getSubmissionChartData(): array
    {
        $submissionQuery = "
            SELECT
        ...
        ";
        $result = $this->conn->query($submissionQuery);
        $labels = [];
        $onTime = [];
        $late = [];
        ...
        return ["labels" => $labels, "onTime" => $onTime, "late" => $late];
    }

    public function getProgressChartData(): array
    {
        $progressQuery = "SELECT status, COUNT(*) total FROM milestones GROUP BY status";
        $result = $this->conn->query($progressQuery);
        $progress = [
            "in-progress" => 0,
            "in-review" => 0,
            "approved" => 0,
        ...
        return ["labels" => array_keys($progress), "values" => array_values($progress)];
    }

    public function getAdviserWorkload(): array
    {
        $workloadQuery = "
            SELECT
        ...
        ";
        $result = $this->conn->query($workloadQuery);
        $labels = [];
        $groups = [];
        while ($row = $result->fetch_assoc()) {
            $labels[] = $row['adviser'];
    ...
    public function getRecentActivity(): array
    {
        $activityQuery = "
            ...
            LIMIT 6
        ";
        $result = $this->conn->query($activityQuery);
        $activity = [];
        while ($row = $result->fetch_assoc()) {
            $activity[] = [
    ...
    public function getRecentSubmissions(): array
    {
        $recentSubsQuery = "
            ...
            LIMIT 6
        ";
        $result = $this->conn->query($recentSubsQuery);
        $recentSubs = [];
        while ($row = $result->fetch_assoc()) {
            $recentSubs[] = [
                "id" => (int) $row['id'],
