...
function loadUsers() {
    ...
    fetch(`../configuration/adm-users-data.php?${params.toString()}`)
        .then(res => res.json())
        .then(data => {
...
function renderPagination(total, page, perPage) {
    ...
    document.getElementById("paginationSummary").textContent = `Showing ${start}-${end} of ${total}`;
...
function bindToolbarEvents() {
    ...
    document.querySelectorAll("#roleFilterChips .filter-chip").forEach(chip => {
        chip.addEventListener("click", () => {
            document.querySelectorAll("#roleFilterChips .filter-chip").forEach(c => c.classList.remove("active"));
